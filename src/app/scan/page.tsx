...
const BATCH_INFO: Record<
  BatchSize,
  { label: string; count: string; time: string }
> = {
      ...
      <div className="container mx-auto px-4 py-12 max-w-7xl relative z-10">
        <header className="mb-10 flex items-center justify-between">
          ...
          <Link href="/">
            <Button
              variant="outline"
              className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 hover:border-indigo-500/50 hover:text-white cursor-pointer"
            >
              Back to Analysis
            </Button>
          </Link>
                ...
                {(Object.keys(BATCH_INFO) as BatchSize[]).map((size) => (
                  <button
                    ...
                    className={`p-4 rounded-lg border transition-all duration-200 cursor-pointer ${
                      batchSize === size
                        ...
                        : "border-zinc-700 bg-zinc-800/50 text-zinc-400 hover:border-indigo-500/50 hover:bg-zinc-700/50 hover:text-white"
                    } ${
                      isLoading
                  ...
                  >
                    ...
                    <div className="text-sm opacity-70">
                      {BATCH_INFO[size].count}
                    </div>
                    <div className="text-xs opacity-50 mt-1">
                      {BATCH_INFO[size].time}
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              >
                {isLoading ? (
                  <span className="flex items-center gap-2">
                    ...
                    Scanning {BATCH_INFO[batchSize].count}... (
                    {BATCH_INFO[batchSize].time})
                  </span>
                ) : (
              ...
              {scanData && (
                <div className="flex items-center gap-4 text-sm text-zinc-400">
                  <span>
                    Scanned: {scanData.successfulScans}/{scanData.totalScanned}
                  </span>
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1">
                  {(
                    ["all", "buy", "accumulate", "neutral"] as FilterVerdict[]
                  ).map((f) => (
                    <button
                      key={f}
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {filteredResults.map((result) => (
                      <tr
                        key={result.pair}
                        className={`hover:bg-zinc-800/50 transition-all duration-200 cursor-pointer ${
                          result.data?.verdict === "Strong Buy"
                            ? "bg-emerald-500/5"
                            : result.data?.verdict === "Buy"
                            ? "bg-emerald-500/[0.02]"
                            : ""
                        }`}
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`font-mono font-medium ${
                              (result.data?.compositeScore ?? 0) >= 20
                                ? "text-emerald-400"
                                : (result.data?.compositeScore ?? 0) >= 0
                                ? "text-amber-400"
                                : "text-rose-400"
                            }`}
                          >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-sm ${getTrendStyle(
                              result.data?.trend ?? "",
                            )}`}
                          >
                            {result.data?.trend ?? "-"}
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`font-mono ${getRsiStyle(
                              result.data?.rsi ?? 50,
                            )}`}
                          >
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`font-mono ${
                              (result.data?.potentialGain ?? 0) >= 10
                                ? "text-emerald-400"
                                : "text-zinc-300"
                            }`}
                          >
                        ...
                        <td className="px-4 py-3 text-center">
                          <Link
                            href={`/?pair=${encodeURIComponent(result.pair)}`}
                          >
                            <Button
                              variant="ghost"
                              size="sm"
                              className="text-indigo-400 hover:text-white hover:bg-indigo-600 cursor-pointer transition-all duration-200"
                            >
                              Analyze
                            </Button>
                          </Link>
            ...
            <div className="mt-6 p-4 bg-zinc-900/30 rounded-lg border border-zinc-800/50">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs text-zinc-500">
                <div>
                  <span className="text-emerald-400">RSI &lt; 30</span> =
                  Oversold (potential buy)
                </div>
                <div>
                  <span className="text-emerald-400">ADX &gt; 25</span> = Strong
                  trend
                </div>
                <div>
                  <span className="text-emerald-400">Score &gt; 20</span> = Buy
                  signal
                </div>
                <div>
          ...
          <Card className="bg-zinc-900/30 border-zinc-800/50">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-zinc-600 text-sm">
                The scan analyzes each coin using the same multi-factor system
                as the main analysis page.
              </p>
            </CardContent>
          </Card>
        )}
